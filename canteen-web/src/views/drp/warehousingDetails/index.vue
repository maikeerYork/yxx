...
<!--                  <el-form-item label="供应商">
      ...
      <el-table :data="purchaseSummaryList" size="mini" v-loading="loading" :header-cell-style="{'text-align':'center','color':'#000'}">
        <el-table-column prop="apply_no" label="入库单号" />
        <el-table-column prop="raw_no" label="食材编号" />
        <el-table-column prop="raw_name" label="食材名称" />
        <el-table-column prop="in_nums" label="入库数量" />
        ...
        <el-table-column prop="update_at" label="入库时间" />
        <el-table-column prop="username" label="入库人员" />
...
export  default  {
  ...
  methods: {
    ...
    getPurchaseSummaryList() {
      ...
      // axios.get('./warehousingDetails.json').then(res=>{
      //   /*console.log(res)
      //   debugger;*/
      ...
      // })
      getInandOutDetail(obj).then(res=>{
        const{data:{data:{total,data}}} =res
        this.purchaseSummaryList = data
