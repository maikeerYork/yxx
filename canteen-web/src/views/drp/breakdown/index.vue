  ...
  <div>
    ...
    <el-card class="box-shadow-none m-20" :body-style="{ padding: '0' }">
        ...
        <div class="flex-row align-center p-10 justify-between">
          <div class="toggle-box font-size-14">
            <a
              ...
              :title="
                isShowScreen === true ? '点击收起搜索条件' : '点击展开搜索条件'
              "
            >
              筛选
              <i
                :class="
                  isShowScreen === true
                    ? 'el-icon-arrow-down icon-show'
                    : 'el-icon-arrow-down icon-hide'
                "
              ></i>
            </a>
            <span class="ml-5" v-if="!isShowScreen">
              已选条件：
              <el-tag size="small"
                >起止时间：{{ select_date[0] }} - {{ select_date[1] }}</el-tag
              >
            </span>
          </div>
          <div>
                ...
                <el-form size="small" inline>
                  ...
                  <el-form-item label="原料">
                    <div class="flex-row">
                      <div>
                        <el-tag
                          v-if="raw_id_list.length > 0"
                          @close="removeRawId(raw_id_list[0].id)"
                          size="small"
                          closable
                          >{{ raw_id_list[0].raw_name }}</el-tag
                        >
                        <el-popover
                        ...
                        >
                          <div>
                            <el-form inline size="small">
                              <el-form-item
                                v-for="(item, index) in raw_id_list"
                                class="mb-10"
                                v-if="index != 0"
                                :key="item.id"
                              >
                                <el-tag
                                  size="small"
                                  @close="removeRawId(item.id)"
                                  closable
                                  >{{ item.raw_name }}</el-tag
                                >
                              </el-form-item>
                            </el-form>
                          </div>
                          <a slot="reference" class="mr-12" href="javascript:;">
                            <el-tag size="small"
                              >+ {{ raw_id_list.length - 1 }}(查看)</el-tag
                            >
                          </a>
                        </el-popover>
                      </div>
                  ...
                  <el-form-item label="采购人员">
                    ...
                    >
                      <el-option
                        v-for="(item, index) in adminUserList"
                        :key="index"
                        :label="item.username"
                        :value="item.username"
                      />
      ...
      >
        ...
        <el-table-column prop="in_house_at" label="采购日期" />
        <el-table-column prop="apply_no" label="单据编号" />
        <el-table-column prop="category_name" label="采购类型" />
        <el-table-column prop="sup_name" label="供应商名称" />
        ...
        <el-table-column prop="real_nums" label="实际入库数" />
        <el-table-column label="单价">
          <template slot-scope="scope">
            <div class="text-red">￥{{ scope.row.approval_price | price }}</div>
          </template>
        </el-table-column>
        <el-table-column label="金额合计">
          <template slot-scope="scope">
            <div class="text-red">￥{{ scope.row.sum | price }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="whouse_name" label="仓库名称" />
        <el-table-column prop="username" label="申购人员" />
          ...
          <el-col :span="24">
            <el-form size="small" inline>
              ...
              <el-form-item label="仓库">
                ...
                >
                  <el-option
                    v-for="item in rawWareHouseList"
                    :key="item.id"
                    :label="item.whouse_name"
                    :value="item.id"
                  />
              ...
              <el-table-column>
                <template slot="header">
                  <div class="font-size-12 text-black">
                    <el-checkbox
                      @change="changerawCheckedAll()"
                      :indeterminate="
                        this.rawDataList.filter((x) => x.checked === true)
                          .length !== this.rawDataList.length &&
                        this.rawDataList.filter((x) => x.checked === true)
                          .length > 0
                      "
...
import { purchaseDetails } from "@/api/purchaseStatistics";
...
export default {
  ...
  methods: {
    loadChimeePlayer() {
      setTimeout(() => {
        let chiee = new ChimeePlayer({
          ...
          src: "http://171.221.174.110:8999/hdl/live101/102.flv",
          box: "flv",
          isLive: true,
          autoplay: true,
          controls: true,
    ...
    getRawList() {
      ...
      if (this.rawKeyWords !== "")
        Object.assign(obj, { keywords: this.rawKeyWords });
    ...
    getPurchaseDetailList() {
      ...
      if (this.purchaseTypeId !== "")
        Object.assign(obj, { category_id: this.purchaseTypeId });
      if (this.admin_username.length > 0)
        Object.assign(obj, { username: this.admin_username.join(",") });
      ...
      purchaseDetails(obj)
        .then((res) => {
          ...
          console.log("purchaseDetailList", this.purchaseDetailList);
    ...
    // 初始化下载链接
    initDownLoadUrl() {
      this.downloadUrl = `${kang_url}/excel/ware/purchase/detail/export?canteen_id=${
        this.canteenId
      }&
        date_start=${this.select_date[0]}&date_end=${
        this.select_date[1]
      }&house_id=${this.wHouse_id}
        ${this.keywords !== "" ? "&apply_no=" + this.keywords : ""}
        ${this.supplierId !== "" ? "&supplier_id=" + this.supplierId : ""}
        ${
          this.purchaseTypeId !== ""
            ? "&category_id=" + this.purchaseTypeId
            : ""
        }
        ${
          this.admin_username.length > 0
            ? "&username=" + this.admin_username.join(",")
            : ""
        }
        ${
